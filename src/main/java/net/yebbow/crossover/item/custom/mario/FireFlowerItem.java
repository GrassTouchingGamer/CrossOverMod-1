...
public class FireFlowerItem extends Item {
    ...
    public InteractionResultHolder use( Level level, Player player, InteractionHand hand) {
        ...
        if (!level.isClientSide) {
            ...
            level.playSound((Player) null, player.getX(), player.getY(), player.getZ(), ModSounds.FIREFLOWER_USE.get(), SoundSource.NEUTRAL, 0.5F, 0.0F / (level.getRandom().nextFloat() * 0.4F + 0.8F));
